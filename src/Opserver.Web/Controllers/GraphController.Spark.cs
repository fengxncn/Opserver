...
public partial class GraphController(
    ...
    IMemoryCache cache,
    IOptions<OpserverSettings> settings) : StatusController(settings)
{
    ...
    private SQLModule Sql { get; } = sqlModule;
    private DashboardModule Dashboard { get; } = dashboardModule;
    private IMemoryCache MemCache { get; } = cache;

    [OnlyAllow(DashboardRoles.Viewer)]
    [ResponseCache(Duration = SparkGraphDuration, VaryByQueryKeys = ["id"], Location = ResponseCacheLocation.Client)]
    ...
    [OnlyAllow(DashboardRoles.Viewer)]
    [ResponseCache(Duration = 120, VaryByQueryKeys = ["id", "iid"], Location = ResponseCacheLocation.Client)]
    ...
    [OnlyAllow(DashboardRoles.Viewer)]
    [ResponseCache(Duration = 120, VaryByQueryKeys = ["id"], Location = ResponseCacheLocation.Client)]
    ...
    [ResponseCache(Duration = SparkGraphDuration, VaryByQueryKeys = ["node"], Location = ResponseCacheLocation.Client)]
