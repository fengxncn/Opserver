...
public sealed class OnlyAllowAttribute(string role) : AuthorizeAttribute, IAuthorizationFilter
{
    public string Role { get; set; } = role ?? throw new ArgumentOutOfRangeException(nameof(role));
...
public sealed class AlsoAllowAttribute(string role) : Attribute
{
    public string Role { get; set; } = role ?? throw new ArgumentOutOfRangeException(nameof(role));
}
