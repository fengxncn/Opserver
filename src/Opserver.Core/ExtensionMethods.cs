...
public static partial class ThirdPartyExtensionMethods
{
    private static readonly Regex _traceRegex = GetTraceRegex();
    ...
    [GeneratedRegex(@"(.*).... \((\d+) more bytes\)$", RegexOptions.Compiled)]
    private static partial Regex GetTraceRegex();
}
...
public static class IntToBytesExtension
{
    ...
    private static readonly List<string> Units = ["", "K", "M", "G", "T"];
