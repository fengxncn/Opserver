...
internal static class Wmi
{
    ...
    internal class WmiQuery : IDisposable
    {
        ...
        public WmiQuery(string machineName, string query, string wmiNamespace = @"root\cimv2", (string Username, string Password) credentials = default)
        {
            ...
            _searcher = _searcherCache.GetOrAdd(path + query, _ => new ManagementObjectSearcher(scope, new ObjectQuery(query), new System.Management.EnumerationOptions { Timeout = connectionOptions.Timeout }));
        }
    ...
    private class WmiDynamic(ManagementObject obj) : DynamicObject
    {
        private readonly ManagementObject _obj = obj;
