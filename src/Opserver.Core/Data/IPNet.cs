...
public class IPNet
{
    ...
    public static IPNet Parse(string ipOrCidr)
    {
        ...
        if (slashPos > -1)
        {
            if (byte.TryParse(span[(slashPos + 1)..], out var cidr))
            {
                return Parse(span[..slashPos], cidr);
            }
    ...
    /// </summary>
    private static readonly List<IPNet> ReservedMulticastRanges =
        [
            Parse("224.0.0.0/4"),
            Parse("ff00::/8"),
        ];
    ...
    /// </summary>
    private static readonly List<IPNet> ReservedLinkLocalRanges =
        [
    ...
    /// </summary>
    private static readonly List<IPNet> ReservedDocumentationRanges =
        [
    ...
    public readonly struct TinyIPAddress : IEquatable<TinyIPAddress>, IComparable<TinyIPAddress>
    {
        ...
        public static TinyIPAddress? FromIPAddress(IPAddress addr)
        {
            ...
            {
#if NETCOREAPP
                return new TinyIPAddress(FromBytes(source), FromBytes(source[8..]));
#else
