...
public partial class BosunDataProvider(DashboardModule module, BosunSettings settings) : DashboardDataProvider<BosunSettings>(module, settings)
{
        ...
        string metricName)
    {
        ...
        var apiResponse = await GetMetric(
            ...
            id);
        return apiResponse?.Series?[0]?.PointData ?? [];
    }
    ...
    public override async Task<List<DoubleGraphPoint>> GetNetworkUtilizationAsync(Node node, DateTime? start, DateTime? end, int? pointCount = null)
    {
        ...
        return JoinNetwork(apiResponse?.Series) ?? [];
    }
