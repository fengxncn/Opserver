...
{
    private static readonly ImmutableHashSet<string> _ignoredTags = ["dsname", "computationId", "plugin", "fs_type", "mountpoint"];
    ...
    private static readonly SignalFlowStatement[] _signalFlowStatements = [
        Cpu,
        Memory,
        DiskUsage,
        DiskUsageByHost,
    ...
    private readonly struct TimeSeries(SignalFxDataProvider.TimeSeriesKey key, ImmutableArray<GraphPoint> values)
    {
        public bool IsDefault => Values.IsDefault;
        public TimeSeriesKey Key { get; } = key;
        public ImmutableArray<GraphPoint> Values { get; } = values;
    }

    private readonly struct TimeSeriesKey(string host, string metric, ImmutableDictionary<string, string> tags) : IEquatable<TimeSeriesKey>
    {
        ...
        public ImmutableDictionary<string, string> Tags { get; } = tags;
        ...
        {
            if (obj is not TimeSeriesKey other)
            {
                return false;
            }
                ...
                else if (msg is DataMessage metricData)
                {
                    foreach (var metricValue in metricData.Values)
                    {
                        if (timeSeriesMap.TryGetValue(metricValue.TimeSeriesId, out var timeSeriesKey))
                        {
                            ...
                            {
                                pointsByHost[timeSeriesKey] = metricValues = [];
                            }
    ...
    private class ConnectMessage(string token) : SignalFlowMessage
    {
    ...
    private readonly struct SignalFlowStatement(string metric, string aggregation = null, string rollup = null)
    {
        public string Metric { get; } = metric;
        public string Aggregation { get; } = aggregation;
        public string Rollup { get; } = rollup;
    ...
    private class SignalFlowClient : IAsyncDisposable
    {
        ...
        public async ValueTask ConnectAsync()
        {
            ...
            var response = await WaitOneAsync<AuthenticatedMessage>() ?? throw new SignalFlowException("Unable to authenticate to SignalFlow endpoint.");
        }
