...
public class JiraClient(JiraSettings jiraSettings)
{
    private readonly JiraSettings _jiraSettings = jiraSettings;
    private static readonly HashSet<string> HiddenHttpKeys =
    [
            "ALL_HTTP",
            "ALL_RAW",
            "HTTP_CONTENT_LENGTH",
    ...
    private static readonly HashSet<string> DefaultHttpKeys =
    [
        "APPL_MD_PATH",
        "APPL_PHYSICAL_PATH",
        "GATEWAY_INTERFACE",
    ...
    public async Task<JiraCreateIssueResponse> CreateIssueAsync(JiraAction action, Error error, string accountName)
    {
        ...
        var labels = action.Labels.IsNullOrEmpty()
            ? null
            : action.Labels.Split(separator, StringSplitOptions.RemoveEmptyEntries);
